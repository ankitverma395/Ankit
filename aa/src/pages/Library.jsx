...
const Library = () => {
  ...
  useEffect(() => {
    if (!token) return; // ✅ FIX: wait for token
    fetchPlaylists();
  }, [token]);
  ...
  const fetchPlaylists = async () => {
    try {
      ...
      setError('Failed to load playlists'); // ✅ added
    } finally {
      setLoading(false);
    }
  };
  ...
  const createPlaylist = async () => {
    ...
    try {
      ...
      setPlaylists(prev => [...prev, res.data]); // ✅ FIX
      setNewPlaylistName('');
      setShowCreateModal(false);
    } catch (err) {
      console.error('Error creating playlist:', err);
      setError('Failed to create playlist'); // ✅ added
    }
  };
  ...
  const deletePlaylist = async (playlistId) => {
    ...
    try {
      ...
      setPlaylists(prev => prev.filter(p => p._id !== playlistId)); // ✅ FIX
    } catch (err) {
      console.error('Error deleting playlist:', err);
      setError('Failed to delete playlist'); // ✅ added
    }
  };
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <motion.div className="glass-card p-6 w-full max-w-md">
            ...
            <input
              ...
              autoFocus
              onKeyDown={(e) => e.key === 'Enter' && createPlaylist()} // ✅ UX fix
            />
